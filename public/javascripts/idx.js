...
let onUpdateQty = () => {
  ...
  if (selectedProduct && selectedProduct.id && Number(remaining)) {
    postReq('/update-product/', { id: selectedProduct.id, remaining, qty: true })
      .then(res => {
        console.log(res);
        if (res.success) {
          console.log('OK');
...
let onUpdateProduct = () => {
  ...
  if (name.length || category.length || shelf.length ||
    ...
    // send request
    postReq('/update-product/', { id: selectedProduct.id, remaining, edit: true })
      .then(res => {
...
let onSaveNewProduct = () => {
  ...
  // todo: validate data
  if (product.name.length || product.category.length || product.shelf.length ||
    Number(product.remaining) || Number(product.watch_qty) || Number(product.pack_qty) ||
    Number(product.cost) || Number(product.selling) || Number(product.priority)) {
    console.log('qas');
